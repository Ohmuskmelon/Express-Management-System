...
void user_sign_up (user user_data, int *user_num, int user_id, int password, thread_data *t_data)
{
    ...
    ret = send(t_data->conn_fd, t_data->send_buf, sizeof(receive_message), 0);
    if (ret < 0)
        thread_err("发送数据出错！\n", t_data->buff_index);
}
...
/* 用户取出所有快递 */
void get_all_expresses_out(express exp_data, int exp_num, int exp_index, user user_data, int user_num, int user_id, int user_list_num, int *list, thread_data *t_data)
{
    for (int i=0; i<user_list_num; i++)
